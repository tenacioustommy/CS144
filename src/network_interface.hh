...
private:
  ...
  // numeric ipv4 to ethernetaddress
  std::unordered_map<uint32_t, EthernetAddress> arpcache {};
  bool timer_on = false;
  size_t cul_time = 0;
  std::vector<std::pair<uint32_t, uint32_t>> maptimeout {};
  std::queue<InternetDatagram> ip_datagram {};
  uint32_t waiting_ip = 0;
  std::queue<EthernetFrame> frame_queue {};

public:
  // Address get_addr() { return ip_address_; }
